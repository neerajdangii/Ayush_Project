(function () {
  ...
  // ─── KEY FIX: tail blocks are collected as-is (not unwrapped),
  //              keep-together nodes inside tail are kept together,
  //              but tail itself is allowed to be placed inline after table
  //              rather than always forced to a new page.
  ...
  function collectPageItems(source, tail) {
    ...
    function normalize(node) {
      ...
      // FIX: For tail blocks, unwrap them but keep their children together
      // so they can be placed on the same page as the last table row
      // if space allows, instead of being treated as one giant keep-together block.
      if (isTailBlock(node)) {
        // Check if the tail block also has coa-keep-together
        // If so, treat it as a single keep-together unit but still allow
        // it to flow onto the current page naturally.
        if (node.classList && node.classList.contains("coa-keep-together")) {
          // Push as a single block — but DON'T force a new page before it.
          // The pagination loop will place it on current page if it fits,
          // or move to next page only if it genuinely doesn't fit.
          items.push(node.cloneNode(true));
          return;
        }
        // Unwrap tail block children individually
  ...
  function splitTableNode(page, container, sourceTable, pageHasContent) {
    ...
    sections.forEach(function (section, sectionIndex) {
      ...
      rowGroups.forEach(function (group, groupIndex) {
        ...
        if (isOverflowing(page, container)) {
          // Always remove the overflowing rows — never let a row bleed past the footer
          appendedGroupRows.forEach(function (rowClone) {
            if (rowClone.parentNode) rowClone.parentNode.removeChild(rowClone);
          });

          // Move all remaining rows (this group + subsequent groups) to remainder
          group.forEach(function (row) {
            remainderSections[sectionIndex].appendChild(row.cloneNode(true));
          });
    ...
    if (!fittedRows) {
      // No rows fit — if page is empty, force the first row to avoid infinite loop
      if (!pageHasContent && sections.length && sections[0].rows.length) {
        var wb = workingTable.querySelector("tbody");
        if (!wb) { wb = document.createElement("tbody"); workingTable.appendChild(wb); }
        // Add first row to working table
        wb.appendChild(sections[0].rows[0].cloneNode(true));
        // Remove that same first row from remainder table so it is NOT duplicated
    ...
    // FIX: After splitting, add a visible bottom border to the last row
    // of the working table. We use both border and box-shadow because
    // overflow:hidden clips borders but NOT box-shadow.
    (function () {
  ...
  function appendNode(page, container, node, pageHasContent) {
    ...
    // ─── KEY FIX for keep-together blocks (including the tail END OF REPORT block):
    // Try to fit the whole block on the current page first.
    // Only move to the next page if it genuinely doesn't fit AND the page already has content.
    // This ensures "END OF REPORT" appears right after the last table row when space allows.
    if (
      node.nodeType === Node.ELEMENT_NODE &&
      node.classList &&
      ...
      if (!pageHasContent) {
        // Page is empty — force it on this page anyway to avoid infinite loop
        container.appendChild(node.cloneNode(true));
        return { type: "fit" };
      }

      // Page has content — move to next page
      return { type: "nofit" };
    }
